...
\end{equation}
These procedures make the inverting scaling as $\sim M^2$ instead of $\sim M^3$, which is largely beneficial for complex systems.

Henceforth we assume that we do not have any variational parameter in the determinant, and obtain three expressions of the case when a spin-up electron is moved and three of the case when a spin-down electron is moved, and get the following equation for $k<N_{\uparrow}$:
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\begin{aligned}
...
\end{empheq}
and the following equations for $k\geq N_{\uparrow}$:
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\begin{aligned}
