...
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\Psi_{\text{sd}}(\boldsymbol{r})\propto|\hat{D}_{\uparrow}(\bs{r}_{\uparrow})|\cdot |\hat{D}_{\downarrow}(\bs{r}_{\downarrow})|
\end{empheq}
...
\frac{|\Psi_{\text{sg}}(\bs{x}_{\text{new}})|^2}{|\Psi_{\text{sg}}(\bs{x}_{\text{old}})|^2}=\exp\Big(\omega\alpha\big((x_{i}^{\text{old}})^2-(x_{i}^{\text{new}})^2\big)\Big),
\end{empheq}
...
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\begin{aligned}
\frac{|\Psi_{\text{pj}}(\bs{r}_{\text{new}})|^2}{|\Psi_{pj}(\bs{r}_{\text{old}})|^2}&=\exp\Big(2\sum_{j=1}^Na_{ij}(h_{ij}^{\text{new}}-h_{ij}^{\text{old}})\Big)\\
