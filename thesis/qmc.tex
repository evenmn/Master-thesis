...
\end{equation}
indicating the desire of a large number of cycles. This is associated with a zero-variance property governing the VMC method, stating that the variance in the true ground state should be zero \cite{giuseppe_carleo_neural-network_2017, deb_variational_2014}. Various sampling regimes were described in section \ref{sec:metropolis}.
...
Based on the previous section, it might be hard to recognize the similarities between Gibbs sampling and the Metropolis sampling methods. However, the connection between them is crucial for several reasons. Firstly, this can lead us to the link between standard restricted Boltzmann machines and standard VMC, as Boltzmann machines often are based on Gibbs sampling and VMC often is based on the Metropolis samplings. We need to exploit this link to take full advantage of the Boltzmann machines. Secondly, this might clarify why the Gibbs sampling does not serve our purpose.
