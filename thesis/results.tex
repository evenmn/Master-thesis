...
We are finally ready to discuss the most exciting part of this thesis, namely the results. Since this work is about physics, the physical insights should be our focus and in this chapter we finally get the opportunity to discuss various physical interpretations of the results. Moreover, the various trial wave function ansätze are discussed and compared. The Slater-Jastrow wave function is the \textit{de facto} standard ansatz used in variational Monte Carlo calculations of electronic structure problems. This ansatz, thoroughly discussed in chapter \ref{chp:methods}, is used as reference for our machine learning ansätze. Additionally, it is used for simulations of atomic systems, and will henceforth be abbreviated as the VMC ansatz. We also examine an ansatz consisting of a Slater determinant where the single-particle functions are determined by the marginal distributions of the visible nodes of a Gaussian-binary restricted Boltzmann machine. This is abbreviated as the RBM ansatz. When adding a simple Jastrow factor to this ansatz, we obtain the RBM+SJ ansatz. The RBM+PJ ansatz means that the Padé-Jastrow factor is added to the RBM ansatz. The Jastrow factors were discussed in section \ref{sec:jastrow}. 
