...
\begin{figure}
	...
	\caption{Plots of the radial one-body density profiles, $\rho(r)$, as a function of the radial distance from the nucleus, $r$, multiplied with the radius squared. The Helium atom (upper left), the Beryllium atom (upper right), the Neon atom (lower left) and the Magnesium atom (lower right) are studied using VMC. The number of Monte Carlo cycles used was $M=2^{28}=268,435,456$ and the ADAM optimizer was used. For abbreviations and description of the Hartree atomic units used, see the text.}
