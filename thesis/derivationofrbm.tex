\chapter{Derivation of Gaussian-binary restricted Boltzmann machine} \label{app:rbmderive}
...
The marginal distribution of the hidden units are found by an integral, as the visible units take continuous values. 

\begin{equation}
\begin{aligned}
P(\bs{h})&=\int d\bs{x}P(\bs{x},\bs{h})\\
&=\frac{1}{Z}\int d\bs{x}\exp(-\sum_{i=1}^F\frac{(x_i-a_i)^2}{2\sigma_i^2})\prod_{j=1}^H\exp(h_jb_j+\sum_{i=1}^F\frac{h_jW_{ij}x_i}{\sigma_i^2})\\
&=\frac{1}{Z}\int d\bs{x}\exp(-\sum_{i=1}^F\frac{(x_i-a_i)^2}{2\sigma_i^2}+\sum_{j=1}^Hh_jb_j+\sum_{i=1}^F\sum_{j=1}^H\frac{h_jW_{ij}x_i}{\sigma_i^2})
\end{aligned}
\end{equation}
