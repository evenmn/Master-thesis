from numpy import log2, zeros, mean, var, sum, loadtxt, arange, array, cumsum, dot, transpose, diagonal, floor, ceil
...
def block(x):
    # preliminaries
    n = len(x); d = int(floor(log2(n))); s, gamma = zeros(d), zeros(d);
    mu = mean(x); t0 = time()
