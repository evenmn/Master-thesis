...
\end{enumerate}
The postulates are taken from \url{http://vergil.chemistry.gatech.edu/notes/intro_estruc/node4.html}.
...
\end{equation}
which saves us from a lot of computations. One can easily see that the spin-part now can be factorized out from each of the determinants. It is also worth to notice that the size of the spin-up determinant is determined by the number of spin-up particles, and it is similar for the spin-down determinant. This means that the we can change the total spin $S$ by adjusting the sizes of the determinants.
