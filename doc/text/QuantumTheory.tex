...
\end{equation}
$\gamma$ is a variational parameter, while $\beta_{ij}$ depends on the spin of the interacting objects in the following way:
\begin{equation}
\label{eq:ajastrow}
a_{ij}=
\begin{cases} 
e^2/8D & \text{if $i,j$ are particles of same spin} \\
e^2/4D & \text{if $i,j$ are particles of opposite spin}\\
e^2/2D & \text{if $i,j$ are electron-nuclear pairs}
\end{cases}
\end{equation}
