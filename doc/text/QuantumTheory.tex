...
\end{equation}
so the nominator is basically the integral over all the probabilities. If the wave function is normalized correctly, this should always give 1. Assuming that is the case, the expectation value can be expressed more elegantly by using Dirac notation,
\begin{equation}
...
The quantum virial theorem was first shown by Vladimir Fock, \cite{fock_bemerkung_1930} and reads
\begin{equation}
2\langle \mathcal{\hat{T}} \rangle = \sum_{i=1}^fn_i\langle \mathcal{\hat{V}}_{i} \rangle
\end{equation}
for $f$ sources of potential energy, where each is on the form
\begin{equation}
