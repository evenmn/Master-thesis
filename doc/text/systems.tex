...
In this chapter, we will present the systems studied in our work. We have already mentioned atomic systems, but what we in the first place will focus on are the quantum dot systems. Initially we will look at the single quantum dots, before we proceed to the double quantum dots and end the section with atoms. As we already have seen, every system are associated with a Hamiltonian which is the energy operator of a system. For all the systems, the corresponding Hamiltonian will be detailed with a description of the .. Additionally, common basis sets are presented which often are solutions of the non-interacting cases. 
...
\label{eq:doublewell}
\end{equation}
with $b$ as the distance between the wells and $a$ as an arbitrary integer. \cite{jelic_double_2012} Setting $a=1$ gives two parabolic wells with a sharp local maximum at $x=0$, while $a=2$ gives a smoother but steeper well. In figure \eqref{fig:doublewell} the potential is plotted for $a=1,2$ and $3$.

\begin{figure}
	\centering
	\input{pgf/doublewell.tex}
	\caption{Double-well potentials plotted with $a=1,2$ and 3, and $b=2$. For $a=1$ the potential was multiplied with 2 to make it comparable to the others. }
...
\end{equation}
which means that to find the energy of an atom, we need to summarize the energy og all the electrons. 
...
The radial part can be presented as a function of the \textit{associated Laguerre polynomials} or \textit{generalized Laguerre polynomials}, $L_{q}^p(x)$, and reads
\begin{equation}
...
For atomic systems it is also common to use a Hartree-Fock basis, based on for example Gaussian functions. Even though the Gaussian functions do not have the correct shape, an expansion can be fitted pretty well. \cite{hehre_selfconsistent_1969}
