...
\section{Efficiency}
There are many ways to improve the efficiency of a code. Usually, the first rule is to calculate closed-form expressions for the functions as far as it is possible, which we have done in chapter \ref{chp:WFE}. All the optimization for the respective elements are also described there. 

Secondly, computations already performed should \textbf{not} be repeated as far as it is possible.

\subsection{Updating the distance matrix}
