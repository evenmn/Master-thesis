...
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\frac{|\Phi_{sj}(\bs{r}_{\text{new}})|^2}{|\Phi_{\text{sj}}(\bs{r}_{\text{old}})|^2}=\exp\bigg(2\sum_{j'=1}^N\beta_{i'j'}(r_{i'j'}^{\text{new}}-r_{i'j'}^{\text{old}})\bigg)
\end{empheq}
where $i'$ again is the moved particle. The gradient is straight-forward to find, and reads
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\nabla_k\ln\Phi_{sj}=\sum_{j'=1}^N\beta_{k'j'}\frac{x_k-x_j}{r_{k'j'}}
\end{empheq}
...
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\nabla^2\ln\Phi_{sj}=\sum_{k=1}^{F}\sum_{j'=1}^N\frac{\beta_{k'j'}}{r_{k'j'}}\bigg[1-\Big(\frac{x_k-x_j}{r_{k'j'}}\Big)^2\bigg].
\end{empheq}
...
\end{lstlisting}
where $m_radialStep$ is the width of each annulus, above denoted by $d$. In the end, \lstinline|m_particlesPerBin| is printed to file, and we do the normalization when this file is read by a script, see \lstinline|plot_ob_density.py|. For the two-body density, the implementation is very similar, and it will therefore not be detailed here. 
...
\begin{algorithm}
	...
	\While{not converged}{
		...
		\If{myrank==0}{
			...
			$\overline{\text{Egradient}}=\text{gradient}/M$\;
