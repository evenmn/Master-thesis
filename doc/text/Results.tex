...
\end{equation}
which can be verified from the energy splitting. To do this accurately, we better list up the exact energies, which for a two-dimensional quantum dot is given in table \eqref{tab:splitfrequencyQD2D} for some selected frequencies. For more numbers, please look at appendix \ref{chp:energydistribution}. By doing the math, we see that the virial theorem is satisfied for low frequencies, but as the frequency increases the theorem gets more and more off. This is the case for all the methods.
