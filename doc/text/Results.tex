...
\begin{table}
	...
	\begin{tabularx}{\textwidth}{R{2cm}rR{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}} \hline\hline
		&\makecell{\\ $N$ \\ \phantom{=}} & \multicolumn{1}{c}{$\langle \hat{\mathcal{H}}\rangle$} & \multicolumn{1}{c}{$\langle \mathcal{\hat{T}} \rangle$} & \multicolumn{1}{c}{$\langle \mathcal{\hat{V}_{\text{ext}}} \rangle$} & \multicolumn{1}{c}{$\langle \mathcal{\hat{V}_{\text{int}}} \rangle$} \\ \hline \\
...
\subsection{In-medium dots}
Often when it comes to real-world use of quantum dots, they are not in vacuum, but instead in some kind of medium. In those cases, the medium affects the Coulomb interaction such that the electrostatic potential gets finite. To obtain this, replace the standard Coulomb interaction by screened Coulomb interaction which reads
\begin{equation}
\label{eq:screened}
\langle\mathcal{\hat{V}_{\text{int}}}\rangle=
\begin{cases} 
\sum_{i>j}(1/r_{ij})\exp(r_{ij}/\mu_r) & \text{if } r_{ij}>\lambda_D\\
\sum_{i>j}(1/r_{ij}) & \text{otherwise},
\end{cases}
\end{equation}
with $r_{ij}$ still being the distance between two particles $i$ and $j$, $\lambda_D$ is the Debye screening length and $\mu_r$ determines how fast the field should reduce. Both those constants are dependent on the medium.

Since the interaction is reduced, we expect that the virial theorem is no longer satisfied. 
