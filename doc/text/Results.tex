...
\begin{table}
	...
	\begin{tabularx}{\hsize}{llR{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}R{2.2cm}} \hline\hline
		...
		& 0.5 & 275.262(7) & 271.030(4) & - & & - & 269.746(1) \\
		& 1.0 & 452.732(8) & 442.874(4) & - & & - & 442.602(2) \\ \hline\hline
	\end{tabularx}
\end{table}
\end{landscape}
}
...
Another quantity of particular interest is the one-body density. We have produced one-body density plots using a restricted Boltzmann machine (RBM), a restricted Boltzmann machine with simple Jastrow factor (RBM+SJ), a restricted Boltzmann machine with Padé-Jastrow factor (RBM+PJ) and standard variational Monte-Carlo (VMC) for frequencies $\omega=1.0$,$0.5$ and $0.1$ and up to 42 electrons in two dimensions and 40 electrons in three dimensions. The plots for two- and three-dimensional dots can be found in figures (\ref{fig:OB_interaction_2D_1w}-\ref{fig:OB_interaction_2D_01w}) and \ref{fig:OB_interaction_3D} respectively.

First, we observe that the distribution gets broader as the number of particles increases, which is mainly because the particles interact and then push each other longer and longer from the center of the dot. Next, the density plots get sort of a wave form, with more peaks as the number of particles increases. This indicates that there are some radii where the particles are more likely to be found, which substantiate quantum theory when it comes to discrete positions. When we lower the frequency, we expect this effect to be more noticeable, which we will have a closer look at in section \ref{sec:lowfrequencies}.

For frequency $\omega=1.0$, all the methods give similar one-body densities, but RBM tends to over-exaggerate the wave form, which is naturally because of the absent of a Jastrow factor. This over-exaggeration becomes even more significant when the frequency decreases, which probably happens because the interactions are more important as the frequency decreases. 
...
\begin{landscape}
	\begin{figure} [H]%
		\centering
		\captionsetup{width=0.9\hsize}
		\subfloat[RBM, $\omega=0.1$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.100000w/RBM_ADAM_MC2pow28.png}}}
		\subfloat[RBM+SJ, $\omega=0.1$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.100000w/RBMSJ_ADAM_MC2pow28.png}}}
		\subfloat[RBM+PJ, $\omega=0.1$]{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.100000w/RBM_ADAM_MC2pow28.png}}
		\subfloat[VMC, $\omega=0.1$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.100000w/VMC_ADAM_MC2pow28.png}}}
		
		\subfloat[RBM, $\omega=0.5$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.500000w/RBM_ADAM_MC2pow28.png}}}
		\subfloat[RBM+SJ, $\omega=0.5$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.500000w/RBMSJ_ADAM_MC2pow28.png}}}
		\subfloat[RBM+PJ, $\omega=0.5$]{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.100000w/RBM_ADAM_MC2pow28.png}}
		\subfloat[VMC, $\omega=0.5$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/0.500000w/VMC_ADAM_MC2pow28.png}}}\\
		
		\subfloat[RBM, $\omega=1.0$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/1.000000w/RBM_ADAM_MC2pow28.png}}}
		\subfloat[RBM+SJ, $\omega=1.0$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/1.000000w/RBMPJ_ADAM_MC2pow28.png}}}
		\subfloat[RBM+PJ, $\omega=1.0$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/1.000000w/RBMPJ_ADAM_MC2pow28.png}}}
		\subfloat[VMC, $\omega=1.0$]{{\includegraphics[width=5.1cm]{/home/evenmn/VMC/plots/int1/twobody/2D/20P/1.000000w/VMC_ADAM_MC2pow28.png}}}\\
...
In order to test the code, we also decided to run for systems with $N>56$. This has no scientific significance, other than testing how far a VMC code can go. One thing is that the computations get extremely expensive as the number of particle increases, but also the standard error is assumed to increase due to.. This means that we cannot just crack up the wall clock time and wait when studying large systems, at some point the standard error gets insufficiently large and we need to further increase the number of Monte Carlo cycles. The learning rate also needs to be increased as the systems increase, with requires more iterations.  We look at weakly interacting particles, so we will not get any relativistic effects even when adding many particles.
...
\begin{figure}[H]
	...
	\subfloat[2D, 110P]{\includegraphics[scale=0.4]{/home/evenmn/VMC/plots/int1/onebody/2D/72P/1.000000w/ADAM_MC2pow28.png}}\\
