...
The results are listed in table \eqref{tab:sneq0}, where we use a restricted Boltzmann machine with Padé-Jastrow factor (RBM+PJ) and standard variational Monte-Carlo (VMC). For comparison, the Hartree-Fock limit from Ref.\cite{pederiva_diffusion_1999} (HF) and diffusion Monte-Carlo results from Refs.\cite{pederiva_diffusion_1999},\cite{ghosal_incipient_2007} (DMC) are presented. We look at the number of electrons, $N$, which are \textit{not} among the magic numbers in \eqref{eq:HOclosedshell}, and choose the ($L$,$S$) configurations which are possible ground state candidates.
...
\begin{table} [!htbp]
	...
	\begin{tabularx}{\textwidth}{rrXrrXR{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}} \hline\hline
		\makecell{\\ $N$ \\ \phantom{=}} & \makecell{$\omega$} & \phantom{R} & $L$ & $S$ & \phantom{R} & \multicolumn{1}{c}{RBM+PJ} & \multicolumn{1}{c}{VMC} & \multicolumn{1}{c}{\makecell{HF\\ (Ref.\cite{pederiva_diffusion_1999})}} & \multicolumn{1}{c}{\makecell{DMC\\ (Ref.\cite{pederiva_diffusion_1999})$^a$\\ (Ref.\cite{ghosal_incipient_2007})$^b$}} \\ \hline \\
...
\begin{table}
	\caption{ }
	\label{tab:doubleQD}
	\begin{tabularx}{\textwidth}{rrXR{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}} \hline\hline
		\makecell{\\ $N$ \\ \phantom{=}} & $\omega$ & \phantom{R} & \multicolumn{1}{c}{VMC} & \multicolumn{1}{c}{RBM+PJ} & \multicolumn{1}{c}{\makecell{VMC+HF\\ (Ref.\cite{mariadason_quantum_2018})}} & \multicolumn{1}{c}{\makecell{DMC\\ (Ref.\cite{hogberget_quantum_2013})}} \\ \hline \\
		2 & 1.0 && 2.62 & 2.46 & 2.36618(4) & 2.3496(1) \\ \hline \\
		4 & 1.0 && - & - & 7.90232(4) & - \\ \hline \\
		6 & 1.0 && - \\ \hline \\
		8 & 1.0 && - \\ \hline \hline
	\end{tabularx}
\end{table}
