...
To estimate how fast the cost increases as we add more particles, we do linear regression with a function on the form $f(x)=ax^b$ where $x$ is the number of particles while $a$ and $b$ are the unknown parameters to be found. From the limited number of points, we have found the parameters and presented them in table \eqref{tab:cputimefit}.
...
\subsubsection{Closed shell energy}
Utilizing the symmetry of quantum dots of two electrons, M.Taut was able to obtain semi-analytical energies for some specific frequencies $\omega$. More precisely, he found the energy to be $E=3$ for the frequency $\omega=1$ and $E=2/3$ for the frequency $\omega=1/6$ for the two-dimensional case, and $E=2$ for the frequency $\omega=1/2$ and $E=1/2$ for the frequency $\omega=1/10$ for the three-dimensional case. \cite{taut_two_1993}\cite{taut_two_1994}

For other references, we need to rely on what researchers have found before us. Since diffusion Monte-Carlo (DMC) is known to give very accurate results, we will mainly compare our results to J. Høgberget's DMC computations, which exist for closed shell quantum dots of a maximum of 56 electrons in two dimensions and a maximum of 20 particles in three dimensions. \cite{hogberget_quantum_2013}. Comparing the energy to the Hartree-Fock limit is also interesting, mainly because of the Boltzmann machines. We use A.Mariadason's computations for this for quantum dots of a maximum of 20 electrons in two dimensions, and a maximum of 8 particles in three dimensions. \cite{mariadason_quantum_2018} We run some simulations that we could not find any reference for, which means that we cannot ensure that they are correct or not. 

Ground state energy computations of two- and three dimensional closed-shell quantum dots are found in tables \eqref{tab:quantumdotswinteraction2D1} and \eqref{tab:quantumdotswinteraction3D1} respectively. They are performed by a restricted Boltzmann machine (RBM), restricted Boltzmann machine with a simple Jastrow factor (RBM+SJ), restricted Boltzmann machine with Padé-Jastrow factor (RBM+PJ), partly restricted Boltzmann machine (PRBM) and standard variational Monte-Carlo (VMC). In addition, the Hartree-Fock limit (HF) and diffusion Monte-Carlo (DMC) are present for reference purposes. 
...
\begin{table}
	...
	\begin{tabularx}{\hsize}{l:l|R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}} \hline\hline
		...
		12 & 0.1 & 12.705(2) & 12.566(2) & 12.3517(5) & - & 12.9247 & - & 12.3201(3) & 12.26984(8) \\ 
		& 0.28 & 26.389(2) & 26.083(1) & 25.7645(1) & - & 26.5500 & - & 25.7038(4) & 25.63577(9) \\
		& 0.5 & 40.440(3) & 39.694(1) & 39.4549(8) & - & 40.2161 & - & 39.2660(4) & 39.1596(1) \\
		& 1.0 & 67.632(3) & 66.378(2) & 66.047(3) & - & 66.9113 & - & 65.7946(8) & 65.7001(1) \\ \hdashline
	\end{tabularx}
\end{table}
...
\begin{table}
	\begin{tabularx}{\hsize}{l:l|R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}R{2.45cm}} \\
		...
		& 1.0 & 553.40(1) & 549.76(1) & 543.750(3) & - & - & - & 543.738(7) & 542.9428(8) \\ \hdashline \\
...
\subsubsection{Open shell energy}
For open shell quantum dots, analytical results are absent, but there exist several experimental results. We will mainly benchmark our computations against Ref.\cite{pederiva_diffusion_1999} and Ref.\cite{ghosal_incipient_2007}, who have performed DMC- and HF computations for various spin configurations. 

The results are listed in table \eqref{tab:sneq0}, where we use a restricted Boltzmann machine with Padé-Jastrow factor (RBM+PJ) and standard variational Monte-Carlo (VMC). For comparison, the Hartree-Fock limit (Ref.\cite{pederiva_diffusion_1999}) and diffusion Monte-Carlo results (Ref.\cite{pederiva_diffusion_1999},\cite{ghosal_incipient_2007}) are presented. We look at number of electrons, $N$, which are \textit{not} among the magic numbers in \eqref{eq:HOclosedshell}, and choose the ($L$,$S$) configurations which are possible ground state candidates.

We find both the RBM+PJ and VMC to reproduce the reference energy in all the cases. As all sublevels get singly occupied before one gets doubly occupied, the results are consistent with Hund's (first) rule.  


\begin{table}
	\caption{This table presents the ground state energy of $N$ interacting particles for a given spin configuration ($L$,$S$). The results were obtained by a restricted Boltzmann machine with Padé-Jastrow factor (RBM+PJ), standard variational Monte-Carlo (VMC), restricted Hartree-Fock (HF) and diffusion Monte-Carlo (DMC). Energies denoted with $a$ are taken from Ref.43, while the $b$ energies are taken from Ref.44. All energies are given in units of $\hbar$.}
	\label{tab:sneq0}
	\begin{tabularx}{\textwidth}{rrXrrXR{2.3cm}R{2.3cm}R{2.3cm}R{2.3cm}} \hline\hline
		\makecell{\\ $N$ \\ \phantom{=}} & \makecell{$\omega$} & \phantom{R} & $L$ & $S$ & \phantom{R} & \multicolumn{1}{c}{RBM+PJ} & \multicolumn{1}{c}{VMC} & \multicolumn{1}{c}{\makecell{HF\\ (Ref.\cite{pederiva_diffusion_1999})}} & \multicolumn{1}{c}{\makecell{DMC\\ (Ref.\cite{ghosal_incipient_2007})}} \\ \hline \\
		4 & 0.28 && 0 & 1 && 3.7475(2) & 3.7711(5) & 3.9033 & 3.7135(3)\\ \\
		9 & 0.28 && 0 & 3/2 && 15.886(4) & 15.812(4) & 16.1544 & 15.4784(7)\\
		&3.0 && 0 & 3/2 && 97.164(4) & 96.936(7) & - & 97.0095(3)\\ \hline\hline
	\end{tabularx}
\end{table}
...
\begin{landscape}
	...
	\begin{figure} [H]%
		...
		\subfloat[RBM+SJ, 2P]{{\includegraphics[width=4cm]{Images/example.png}}}
		...
		\subfloat[RBM+SJ, 6P]{{\includegraphics[width=4cm]{Images/example.png}}}
		...
		\subfloat[RBM+SJ, 12P]{{\includegraphics[width=4cm]{Images/example.png}}}
	...
	\begin{figure} [H]%
		...
		\subfloat[RBM+SJ, 20P]{{\includegraphics[width=4cm]{Images/example.png}}}
		...
		\subfloat[RBM+SJ, 30P]{{\includegraphics[width=4cm]{Images/example.png}}}
		...
		\subfloat[RBM+SJ, 42P]{{\includegraphics[width=4cm]{Images/example.png}}}
	...
	\begin{figure} [H]%
		...
		\subfloat[RBM+SJ, 56P]{{\includegraphics[width=4cm]{Images/example.png}}}
