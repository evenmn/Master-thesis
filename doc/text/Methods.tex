...
If we denote $\bs{r}$ as the current state, and $\bs{r}'$ as the proposed state, we have a transition rule $\mathcal{P}(\bs{r}\rightarrow\bs{r}')$ for going from $\bs{r}$ to $\bs{r}'$ and a transition rule $\mathcal{P}(\bs{r}'\rightarrow\bs{r})$ for going the other way around. If we then assume that the rules satisfy \textit{ergodicity} and \textit{detailed balance}, we have the following relationship:
\begin{equation}
\mathcal{P}(\bs{r}\rightarrow\bs{r}')P(\bs{r})=\mathcal{P}(\bs{r}'\rightarrow\bs{r})P(\bs{r}').
\end{equation}
...
\begin{equation}
\mathcal{P}(\bs{r}\rightarrow\bs{r}')=T(\bs{r}\rightarrow\bs{r}')A(\bs{r}\rightarrow\bs{r}').
\end{equation}
In order to satisfy the detailed balance, we need to choose $A(\bs{r}\rightarrow\bs{r}')$ such that
\begin{equation}
A(\bs{r}\rightarrow\bs{r}')=\text{min }\left[1,\frac{T(\bs{r}'\rightarrow\bs{r})P(\bs{r}')}{T(\bs{r}\rightarrow\bs{r}')P(\bs{r})}\right],
\label{eq:acceptance}
\end{equation}
...
\bs{r}'=\bs{r}+sd\bs{r}
\end{equation}
where $s$ is a random number which determines the distance to move and $d\bs{r}$ is a random direction (typically which particle to move). We obtain the naive acceptance probability when setting $T(\bs{r}\rightarrow\bs{r}')=T(\bs{r}'\rightarrow\bs{r})$, such that the it simplifies to
\begin{equation}
A(\bs{r}\rightarrow\bs{r}')=\text{min }\left[1,\frac{P(\bs{r}')}{P(\bs{r})}\right].
\end{equation}
...
\end{equation}
which is called Green's functions. The acceptance probability for importance sampling can finally be written as
\begin{equation}
A(\bs{r}\rightarrow\bs{r}')=\text{min }\left[1,\frac{G(\bs{r},\bs{r}',\Delta t)P(\bs{r}')}{G(\bs{r}',\bs{r}, \Delta t)P(\bs{r})}\right],
\end{equation}
where the probability is still given by equation \eqref{eq:probvmc}. As a summary, we set up the actual algorithm, known as Metropolis-Hasting's algorithm, see algorithm \eqref{al:hasting}.
...
In the machine learning community, Gibbs' sampling is widely used when it comes to training Boltzmann machines. Unlike Metropolis sampling, Gibbs' sampling does not rely on the marginal probabilities, but it actually uses the conditional probabilities to update both the visible and hidden nodes. 
