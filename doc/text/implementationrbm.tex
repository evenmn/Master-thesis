...
An obvious optimization concerning this element, is that we can introduce a vector $\bs{v}=\bs{x}-\bs{a}$, which we deal with instead of the position vector $\bs{x}$ and the parameter vector $\bs{a}$. h
