...
\begin{figure}[H]
	...
	\includegraphics[scale=1]{Images/entanglement.jpeg}
	\caption{The first ever image of quantum entanglement, where two groups of particles are sharing the physical state $\Psi(\bs{r})$, was published in July 2019. Published in Science Advances, Vol. 5, no.7 (2019), \textit{Imaging Bell-type nonlocal behavior} \cite{moreau_imaging_2019}.}
...
The potential, on the other hand, is obviously dependent on the system we want to study. For atomic systems, the potential as a function of the distance from the nucleus can be found from Coulomb's law and reads 
\begin{equation}
V(r)=\frac{1}{4\pi\epsilon_0}\frac{Ze^2}{r}
\label{eq:atompotential}
\end{equation}
where the nucleus is assumed to be at rest in the origin, $k_e=1/4\pi\epsilon_0$ is the Coulomb constant, $Z$ is the atomic number of the nucleus and $e$ is the elementary charge. For a general potential, the Hamiltonian can be expressed as 
\begin{equation}
...
\begin{equation}
\hat{\mathcal{H}}\cdot\frac{k_e^2\hbar^2}{m_eZe^2}=-\frac{1}{2}a_0^2\nabla^2-\frac{1}{a_0r}
\end{equation}
where we have multiplied all the terms by the factor $k_e^2\hbar^2/m_eZe^2$ in order to make the equation dimensionless. We can further scale the energy as $E\leftarrow E\cdot k_e^2\hbar^2/m_eZe^2$ and the distance as $r\leftarrow a_0r$, which both are dimensionless, and obtain the Hamiltonian
\begin{equation}
\hat{\mathcal{H}}=-\frac{1}{2}\nabla^2-\frac{1}{r}
\end{equation}
where all quantities are in atomic units.

The Hydrogen wave function is well-known and reads
\begin{equation}
\Psi(r)\propto e^{-r}
\end{equation}
in atomic units. The bounding energy in Hydrogen is found from the Schrödinger equation in equation \eqref{eq:Energy}, and gives
\begin{equation}
