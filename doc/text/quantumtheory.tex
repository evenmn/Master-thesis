...
Historical recap or directly to Erwin S?

Pioneers though the theory was wrong, Erwin, most accurate theory existing, today accepted by most of the physicists. 
...
We have already mentioned the Schrödinger equation, which describes the mechanics of a quantum mechanical system analogous to Newton's second law for classical mechanics. In this section, we discuss how the equation can be solved and the challenges related to the solutions. As we will see, every observable in quantum mechanics is associated with an operator, and the consequence is that all obtained values are identified with standard errors which are also important to specify in order to present the accuracy of the value. The fundamental quantum numbers that  We will also look at the fundamental quantum numbers which 

The quantum mechanics will make up the framework for this work, but in this chapter we only discuss the theory needed for the work and this is therefore not meant as an encyclopedia to quantum mechanics. For a complete introduction to the topic, Ref. \cite{griffiths_introduction_2005} serves as an excellent read. 
...
\begin{enumerate}
	...
	\item \textit{"In any measurement of the observable associated with an operator $\hat{\mathcal{O}}$, the only values that will ever be observed are the eigenvalues $o$ which satisfy $\hat{\mathcal{O}}\Psi(\bs{r},t)=o\Psi(\bs{r},t)$."}
	
	\item \textit{"The expectation value of the observable corresponding to operator $\hat{\mathcal{O}}$ is given by
		$$\langle\mathcal{O}\rangle=\frac{\int d\tau\Psi^*(\bs{r},t)\hat{\mathcal{O}}\Psi(\bs{r},t)}{\int d\tau\Psi^*(\bs{r},t)\Psi(\bs{r},t)}.\textit{"}$$}
...
\end{enumerate}
All the postulates will be used in this work, and they will be described in detail when we need it. Albeit we will look at stationary states only, even the time-dependent Schrödinger from postulate number 5 will be discussed due to the description of the diffusion Monte Carlo method in section \ref{sec:dmc}. The other will be covered in the current chapter. The formulation of the presented postulates are taken from \cite{sherrill_david_postulates_2003}.

\section{The Schrödinger equation} \label{subsec:schrodinger}
...
\end{equation}
which indicates that the momentum and the position do not \textit{commute}. In other words, the order of the operators in an equation is not arbitrary. 

For systems of two or more charged particles, the potential energy can be split in an external part and an interaction part, where the latter is given by the Coulomb interaction. For two identical particles of charge $q$, the repulsive interaction gives the energy
\begin{equation}
...
In equation \eqref{eq:energyintegral}, we found the expectation value of the energy using the Hamiltonian, which is the energy operator. However, this relation applies for an arbitrary operator $\hat{O}$ related to an expectation value $\langle \hat{O}\rangle$,
\begin{equation}
\langle \hat{O}\rangle=\frac{\int d\bs{r}\psinc\hat{O}\psin}{\int d\bs{r}\psinc\psin},
\label{eq:generalexp}
\end{equation}
which is in agreement with the expected value of the classical absolutely continuous case. As a consequence, there is always an uncertainty associated with a measurement in quantum physics, such that we can only tell the probability of measuring something. The uncertainty is usually described by the variance, which for a set of independent measurements is given by
\begin{equation}
...
The general expected value in equation \eqref{eq:generalexp} is expressed using the Schrödinger wave mechanics picture, but it can also be written in terms of for instance the Heisenberg picture applying matrices. However, the standard formalism today in most of the branches of the quantum mechanics is the Dirac notation, which relates the Schrödinger picture and the Heisenberg picture in an elegant way. Using this, the expectation value reads
\begin{equation}
...
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\varepsilon_0\leq\mel{\Psi}{\hat{\mathcal{H}}}{\Psi}
...
\begin{equation}
\langle\hat{T}\rangle=-\frac{1}{2}\sum_{i=1}^N\langle\bs{F}_i\cdot\bs{r}_i\rangle
\end{equation}
where $\bs{F}_i$ represents the force on particle $i$ at position $\bs{r}_i$. If we further assume that all the sources of potential are on the form $V_i=ar^{n_i}$, we can use the relation $\bs{F}_i=-\nabla V_i$ to express the virial theorem in a simpler fashion
\begin{equation}
2\langle \hat{T} \rangle = \sum_{i}n_i\langle \hat{V}_{i} \rangle,
\end{equation}
which by Vladimir Fock was proven to be true for quantum mechanics in a 1930 paper \cite{fock_bemerkung_1930}. The assumption raised above holds for our situation as the interaction energy goes as $r^{-1}$. We will also later see that the potential used in quantum dots goes as $r^2$ and the potential used in atoms goes as $r^{-1}$ as well.
...
Consider a system of two identical fermions with SPFs $\psi_1$ and $\psi_2$ at positions $\boldsymbol{r}_1$ and $\boldsymbol{r}_2$ respectively. The way we define the wavefunction of the system is then
\begin{equation}
\begin{aligned}
\Psi_T(\bs{r}_1,\bs{r}_2, \sigma_1,\sigma_2)&=\frac{1}{\sqrt{2}}
\begin{vmatrix}
...
\end{vmatrix}
\end{equation}
where the $\psi$'s are the true single particle functions, which are the tensor products 
\begin{equation}
\psi=\phi\otimes\chi
\end{equation}
with $\psi$ as the spatial part and $\chi$ as the spin part. For bosonic systems, one can correspondingly construct a Slater permanent. The permanent of a matrix is similar to the determinant, but all negative signs are replaced by positive signs. 
