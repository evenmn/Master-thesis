...
\end{empheq}
where $\bs{F}_i$ represents the force on particle $i$ at position $\bs{r}_i$. If we further assume that all the sources of potential are in the form of $V_i=ar^{n_i}$, we can use the relation $\bs{F}_i=-\nabla V_i$ to express the virial theorem in a simpler fashion
\begin{equation}
