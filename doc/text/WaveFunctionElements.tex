...
\end{equation*}
$x_{\uparrow}$ are the coordinates of particles with spin up (defined as the first $N_{\uparrow}$ coordinates) and $x_{\downarrow}$ are the coordinates of particles with spin down (defined as the last $N_{\downarrow}$ coordinates). 
...
\begin{equation*}
\ln\psi_{sd}=\ln|\hat{D}_{\uparrow}(\bs{x}_{\uparrow})|+\ln|\hat{D}_{\downarrow}(\bs{x}_{\downarrow})|
\end{equation*}
...
\begin{equation*}
\hat{D}\equiv \hat{D}_{m_s}(\bs{x}_{m_s})
\end{equation*}
...
HERE I NEED TO EXPLAIN HOW TO FURTHER UPDATE SLATERS
---------
...
\frac{|\psi_{sd}(\bs{x}_{\text{new}})|^2}{|\psi_{sd}(\bs{x}_{\text{old}})|^2}&=
...
The Hydrogen-like orbitals were presented in \eqref{eq:hydrogenlike}, but as we discussed they cause some problems for atoms of the size of Neon and larger due to complex numbers. Instead, we decided to look at hydrogen-like orbitals with solid harmonics. Even though they do not have problem with complex numbers, they are quite complicated to differentiate, and the closed form will therefore be found by differentiating numerically. However, we will do the exercise for the simplest case, which is sufficient to find the Hydrogen and Helium ground states. This reads
\begin{equation}
