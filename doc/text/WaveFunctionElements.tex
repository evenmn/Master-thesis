...
\frac{|\psi_{\text{SG}}^{\text{new}}|^2}{|\psi_{\text{SG}}^{\text{old}}|^2}=\exp\Big(\omega\alpha(x_{i,\text{old}}^2-x_{i,\text{new}}^2)\Big)
\end{empheq}

The gradient with respect to coordinate $x_k$ is
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\nabla_k\ln\psi_{\text{SG}}(\alpha)=-\omega\alpha x_k
\end{empheq}
and the corresponding Laplacian is
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\nabla^2\ln\psi_{\text{SG}}(\alpha)=-ND\omega\alpha.
\end{empheq}
...
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\frac{|\psi_{\text{SJ}}^{\text{new}}|^2}{|\psi_{\text{SJ}}^{\text{old}}|^2}=\exp\Big(2\sum_{j=1}^N\beta_{ij}(r_{ij}^{\text{new}}-r_{ij}^{\text{old}})\Big)
\end{empheq}
...
\frac{|\psi_{\text{PJ}}^{\text{new}}|^2}{|\psi_{\text{PJ}}^{\text{old}}|^2}&=\exp\Big(2\sum_{j'=1}^N\beta_{i'j'}(h_{i'j'}^{\text{new}}-h_{i'j'}^{\text{old}})\Big)\notag\\
...
\partial_{\alpha}\ln\Psi &= \frac{x_k-a_k}{\sigma^2}\notag
\end{empheq}
