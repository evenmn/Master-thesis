...
\end{equation}
such that the weight update can be expressed in a neater form
\begin{empheq}[box={\mybluebox[5pt]}]{align}
\frac{\partial\mathcal{C}(\bs{w})}{\partial w_{jk}^{(l)}}=\delta_j^{(l)}a_k^{(l-1)}
\end{empheq}

For an arbitrary layer $l$, we find $\delta_j^{(l)}$ to be de same as about,
\begin{equation}
\delta_j^{(l)}=\frac{\partial \mathcal{C}(\bs{w})}{\partial z_j^{(l)}}=\sum_k\frac{\partial\mathcal{C}}{\partial z_k^{(l+1)}}\frac{\partial z_k^{(l+1)}}{\partial z_j^{(l)}}
\end{equation}
using the definition of $z_j^{(l+1)}$, we obtain the final expression
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
\delta_j^{(l)}=\sum_k\delta_k^{(l+1)}w_{kj}^{(l+1)}f'(z_j^{(l)})
\end{empheq}
where we use the expression for $\delta_j^{(L)}$ as our initial condition. The new weight $w_{ij}^{(l)+}$ is found from
\begin{empheq}[box={\mybluebox[5pt]}]{align}
