...
As hinted above, in machine learning we want to fit a model to a data set in the best possible way. In supervised learning, we have prior knowledge about what kind of results the model should give in some specific cases, which we can use to train our model. After training, the model should be able to reproduce the prior known results, but more important, we also want the model to produce reasonable results for cases where we do not know exactly what the results should be. A basic example is polynomial regression.
...
The estimator vector $\bs{\beta}$ can then be written as a function of the pseudoinverse of $\bs{X}$,
\begin{equation}
\bs{\beta}=\bs{X}^+\bs{y}
\end{equation}
where $\bs{X}^+=\bs{X}^T/\bs{X}^T\bs{X}$. 
