...
\end{equation}
where $\beta_j^+$ is the updated version of $\beta_j$  Here we again are introduced to a hyperparameter $\eta$, known as the \textit{learning rate}, which controls how much the estimators should be changed for each iteration. This should be set carefully, where a too large $\eta$ will make the cost function diverge and a too small $\eta$ will make the training too slow. Typically, the $\eta$ is 0.01-0.0001.


\subsection{Logistic regression}
Up to this point, we have discussed regression with continuous output. But what do we do if we want a discrete output, for example in form of classification?

Need to introduce log likelyhood here
