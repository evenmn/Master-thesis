...
\begin{enumerate}
	...
	\item be able to fit future observations well.
\end{enumerate}
In this section we will take a closer look at how to find a model which satisfies both these goals. If the first one is satisfied, the second does not necessarily be satisfied. Let us first look at polynomial regression. 
...
\end{equation}
and which we seek to minimize. Minimizing this function with respect to $a$ gives
\begin{align}
\frac{\partial \mathcal{C}(a,b,c)}{\partial a} &=\frac{\partial}{\partial a}\bigg(\sum_{i=0}^{n-1}\Big(y_i-(ax_i^2+bx_i+c)\Big)^2\bigg)\notag\label{eq:gh}\\
&=2\sum_{i=0}^{n-1}x_i^2\Big(y_i-(ax_i^2+bx_i+c)\Big)\\
&=0\notag
\end{align}
and similar expressions are obtained for $b$ and $c$. Those equations could of course be solved with respect to $a$, $b$ and $c$, but...
...
By setting $\tilde{y_i}=f(x_i)$ as above, we obtain the general \textit{ordinary least square} (OLS) cost function
\begin{empheq}[box={\mybluebox[5pt]}]{equation}
	\mathcal{C}(\bs{\beta})=\sum_{i=1}^{n}\Big(y_i-\sum_{j=0}^pX_{ij}\beta_j\Big)^2,\qquad\qquad\qquad\text{OLS}
\end{empheq}
which is minimized when
\begin{equation}
\frac{\partial \mathcal{C}(\bs{\beta})}{\partial\beta_j}=2\sum_{i=0}^{n-1}X_{ij}\Big(y_i-\tilde{y}_i\Big)=0
\end{equation}
for all the estimators $\beta_j$. This can be written on a matrix-vector form as 
\begin{equation}
\bs{X}^T\bs{y}=\bs{X}^T\tilde{\bs{y}}=\bs{X}^T\bs{X}\bs{\beta}
\end{equation}
...
\subsubsection{What characterizes a good model?}
If we now go back to the introduction to this chapter, we listed two points...
