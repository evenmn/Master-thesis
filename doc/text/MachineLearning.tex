...
\begin{equation}
\min_{a,b,c}\sum_{i=0}^{n-1}(y_i-f(x_i;a.b.c))^2.
\end{equation}
There are several ways to do this, but they all are based on the \textit{cost function} (also called the loss function), which yields
\begin{equation}
\mathcal{C}(a,b,c)=\sum_{i=0}^{n-1}\Big(y_i-(ax_i^2+bx_i+c)\Big)^2
\end{equation}
...
In this example, we will see how we in practice fit a polynomial to a data set. The data set chosen is the following
\begin{align*}
\bs{x}&=(2,4,6,7,11,13,16)\\
\bs{y}&=(30,50,60,65,55,40,0)
\end{align*}
which is plotted in figure (\ref{fig:polynomials} a). We then use equation \eqref{eq:polynomialestimators} to find the best fitting first-, second- and sixth order polynomials, and obtain
\begin{align*}
f_{\text{1st}}(x)&=-2.14x+60.87\\
f_{\text{2nd}}(x)&=-x^2+15.74x + 2.51\\
f_{\text{6th}}(x)&=-0.001x^6+0.04x^5-0.90x^4+9.04x^3-47.52x^2+129.74x-98.67\\
\end{align*}
where both are plotted together with the data points in figure (\ref{fig:polynomials} b). What we observe is that the first order polynomial gives a large error, while the second order polynomial provides a almost perfect fit. The sixth order polynomial reproduces the targets very well, but it might not be able to reproduce new points in a very good way. From this we can interpret that a first order polynomial has a too low complexity, we say that it underfits the data set. The sixth order polynomial probably overfits the data, since its shape is kind of unnatural, while the second order polynomial seems to have a correct complexity. We will discuss how we can choose a model of correct complexity in the section on bias-variance tradeoff, but before that we will write some words on general linear regression.
\begin{figure}
	\centering
	...
	\caption{The figure on the left-hand-side presents a small number of points, where the figure on the right-hand-side illustrates how a first- and second order polynomial can be fitted to the points in the best possible way.}%
	\label{fig:polynomials}
\end{figure}


\subsection{Linear regression}
Polynomial regression is an example on a linear regression method, and was meant as motivation before we study linear regression in general. Instead of fitting a polynomials to a set of points, we can fit a general function on the form
\begin{equation}
...
The cost function for \textit{the ordinaryordinary least square regression} (OLS) case is already found in equation \eqref{eq:costols}, and we can recall it as
...
\end{empheq}
where we in principle just add the L2-norm of the estimator vector to the OLS cost function. This can easiest be proven going from the cost function to the matrix-vector expression of $\bs{\beta}$.
...
However, to illustrate how this procedure goes on, we will introduce the basic gradient descent method. 
