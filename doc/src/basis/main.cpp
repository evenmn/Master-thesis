...
double NQS_WF(VectorXd Xa, VectorXd v, double sigma) {
    ...
    return exp(-(double)(Xa.transpose() * Xa)/(2 * sigma * sigma)) * prod;
}

int magic_numbers(int n) {
    //Magic numbers of 2D quantum dot.
    //Returns total number of particles in
    //n fully occupied orbits. Starts at 0,
    //following the geometrix series
    //A(n+1) = A(n) + 2^n
...
double Slater(int P, int D, VectorXd Xa, VectorXd v, int sigma) {
    ...
    return D_up.determinant()* D_dn.determinant()*NQS_WF(Xa, v, sigma);
    /*
