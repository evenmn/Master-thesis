...
\note{First I suggest that we take a closer look at the title of my thesis: Studies of Quantum Dots using Machine Learning. We will decompose this, and start with the last term: machine learning.}
...
\note{We now move back to quantum dots, which is the system that we study. We could has studied other systems, such as atoms, but quantum dots are exciting for several reasons. 
...
\note{I will give a breif introduction to the essential quantum theory.}
...
\note{The variational principle is used by variaitonal methods to obtain the ground state energy. Loosely speaking, it states that by solving this integral with an arbitrary wave function, $\Psi_T$, the obtained energy is always larger or equal to the ground state energy. This means that we can estimate the ground state energy by minimizing the obtained energy.}
...
\note{Similar to the theory, also the methods will be discussed briefely.}
...
\note{This makes it on the form of a general expectation value, which straightforward can be approached using Monte Carlo integration, shown here with $M$ Monte Carlo cycles. This does only give an energy, but by adjusting the trial wave function, $\Psi_T$, in order to minimize the energy and repeat the exercise multiple times, we may estimate the ground state energy. }
...
\note{...and are traditionally taylored to every particular system. For quantum dots, the Hermite functions are often used as the single-particle functions. An important finding, is that the gaussian part (point) can be factorized out from the Slater determinant such that the determinant no longer contains variational parameters. Our contribution is to let a restricted Boltzmann machine control the single-particle function...}
...
\note{...as suggested by Carleo and Troyer. }
...
\note{We have decided to study two different Jastrow factors. The simple Jastrow factor does only contain variational parameters multiplied with the relative distances between the particles, $r_{ij}$, and contains a minimum of physical intuition. It is interesting since we want to develop a method that does not require a significant amount of physical intuition. The Padé-Jastrow factor is, on the other hand, taylored to model the electron-electron correlations correctly, and contains therefore more physical information. }
...
\note{Based on this, we have four wave function ansätze. In the following, we will compare their performance in the form of results. }
